...
$query_users = "SELECT registrasi.*, user.nm_user 
                FROM registrasi 
                INNER JOIN user ON registrasi.id_user = user.id_user 
                WHERE registrasi.id_registrasi = $id_users";
...
if (isset($_POST["submit"])) {
    ...
    // Validasi file KTP jika ingin menggantinya
    if (!empty($_FILES['ktp']['name']) && !empty($_FILES['ktp']['tmp_name'])) {
        ...
        // Jika tidak ingin mengganti KTP, gunakan KTP yang ada
        $ktp_file_name = $row_users["foto_ktp"];
    }

    // Validasi file bukti registrasi jika ingin menggantinya
    if (!empty($_FILES['bukti_registrasi']['name']) && !empty($_FILES['bukti_registrasi']['tmp_name'])) {
        ...
        // Jika tidak ingin mengganti bukti registrasi, gunakan bukti registrasi yang ada
        $bukti_registrasi_file_name = $row_users["bukti_registrasi"];
    }

    $query = "UPDATE registrasi SET     
    foto_ktp = '$ktp_file_name', 
    bukti_registrasi = '$bukti_registrasi_file_name', 
    status_regis = '$status_regis'
    WHERE id_registrasi = '$id_users'";
                            ...
                            <div class="card-body px-5 pb-5">
                                ...
                                <form class="row g-3" method="POST" target="" enctype="multipart/form-data">
                                    <div class="col-12">
                                        ...
                                        <input type="text" class="form-control" name="id_user" id="id_user" value="<?php echo $row_users["nm_user"] ?>" placeholder="Nama Pengguna :" readonly>
                                    </div>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status_regis" id="inlineRadio1" value="1" <?php if ($row_users["status_regis"] == 1) echo 'checked' ?>>
                                            <label class="form-check-label" for="inlineRadio1">Menunggu Terkonfirmasi</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="status_regis" id="inlineRadio2" value="2" <?php if ($row_users["status_regis"] == 2) echo 'checked' ?>>
                                    ...
                                    <div style="text-align: left;">
                                        ...
                                        <span id="ktp-file-name"><?php echo $row_users["foto_ktp"]; ?></span>
                                        <small>File format .JPG .JPEG .PNG with a maximum size of 1 MB</small>
                                    </div>

                                    <!-- Tambahkan input file untuk bukti registrasi -->
                                    <div style="text-align: left;">
                                        <label for="bukti_registrasi" style="text-align: left; margin-right: 10px;">Ubah Data Bukti Registrasi:</label>
                                        ...
                                        <span id="bukti-registrasi-file-name"><?php echo $row_users["bukti_registrasi"]; ?></span>
                                        <small>File format .JPG .JPEG .PNG with a maximum size of 1 MB</small>
                                    </div>
